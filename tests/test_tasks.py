...
class TestStateTransitions:
    ...
    def test_mark_complete_sets_commit_id(self):
        """Complete after processing — commit_id and timestamp are set."""
        task = MemoryTask(task_id="t")
        task._mark_processing()
        task._mark_complete("ctx-abc")
        assert task.status == "complete"
        assert task.commit_id == "ctx-abc"
