...
|----------|----------|-------------|
| `APERTUREDB_KEY` | One of the three groups below | Encoded connection key. Generated by `adb config`. Preferred for production. |
| `APERTUREDB_HOST` | | ApertureDB hostname or IP address. |
| `APERTUREDB_PORT` | | ApertureDB port. Default: `55556` (aperture-nexus default). |
| `APERTUREDB_USER` | | ApertureDB username (used with host/port). |
| `APERTUREDB_PASSWORD` | | ApertureDB password (used with host/port). |
...
1. `APERTUREDB_KEY` — a single encoded key (simplest, recommended)
2. `APERTUREDB_HOST` + `APERTUREDB_PORT` + `APERTUREDB_USER` + `APERTUREDB_PASSWORD`
